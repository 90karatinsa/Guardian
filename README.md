...
- `audio.micFallbacks`, platform anahtarları altında `format` ve `device` bilgilerini içeren fallback listeleri kabul eder. Bir cihaz başarısız olduğunda sonraki aday denenir; yapılandırma dosyası kaydedildiğinde aktif boru hattı durdurulmadan yeni liste devreye girer. Guardian her adayın `format` alanını doğrular; yalnızca `alsa`, `avfoundation`, `dshow`, `pulse` ve `pipewire` değerleri kabul edilir. Farklı veya boş bir format yazıldığında hot reload `config.audio.micFallbacks.<platform>[i].format` uyarısıyla yapılandırmayı reddeder. `events.suppression.rules[].timelineTtlMs` değerini değiştirirseniz guard, sıcak durumda timeline TTL'lerini günceller ve geçmiş event kimliklerini yeni süreye göre prune eder.
- `guardian audio devices --format <alsa|pulse|pipewire|dshow|avfoundation>` komutu, keşif çıktısını seçili backend'e göre filtreler. `--json` bayrağıyla birlikte çalıştırıldığında `format` alanı doğrudan raporlanır; örneğin `pnpm tsx src/cli.ts audio devices --format pulse --json` çıktısı PipeWire/PulseAudio eşlemlerini doğrular.
...
- Aynı yaklaşımı ışık dedektörleri için `video.channels.<kanal>.light.temporalMedianWindow` gibi alanlarda da kullanabilirsiniz. Pipeline yeniden başlatılmadan yeni pencere ve backoff katsayıları `pipelines.ffmpeg.byChannel[kanal].health` özetine yansır. `light.normalHours` aralıkları artık kesirli saatleri (`{ "start": 7.5, "end": 21.25 }` gibi) kabul eder; bu sayede gün doğumu/batımı geçişleri dakikalık hassasiyetle tanımlanabilir ve `metrics.detectors.light.gauges.normalHoursActive` yalnızca belirtilen pencere boyunca 1 değerine geçer.
...
- Dashboard sağ panelindeki uyarı listesi, bastırma timeline TTL prune hatalarını `type: "suppression"` olayları olarak gösterir. `pnpm vitest run -t DashboardSuppressionWarningRender` testiyle doğrulanan bu akışta `/api/events/stream` üzerindeki `warnings` alanında `suppression` tipli payload'lar görüyorsanız suppression kurallarınız TTL sınırına yaklaşmış demektir; konfigürasyonda `timelineTtlMs` değerlerini gözden geçirin.
